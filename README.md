...
| --- | --- | --- | --- |
| `react` | `templates/react-assignment/` | React 과제 | 컴포넌트 기반 구조, `src/` + 기본 앱 템플릿 |
| `vanilla` | `templates/vanilla-assignment/` | Vanilla JS 과제 | 번들러 의존 최소화, `src/` + 순수 JS 템플릿 |
...
```bash
node scripts/scaffold.mjs --preset=react --output=generated
node scripts/scaffold.mjs --preset=react,vanilla --output=generated --force
```

- `--preset`: `react`, `vanilla`, `all`(기본값)
- `--output`: 생성 루트 디렉터리(기본값 `generated`)
- `--force`: 대상 폴더가 있어도 덮어쓰기
